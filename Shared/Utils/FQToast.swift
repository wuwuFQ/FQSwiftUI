...
class FQToast {
    fileprivate class func parentView() -> UIView {
        if let view = FQKeyWindow {
            return view
        }
        if let view = FQCurrentController?.view {
            return view
        }
        return UIView()
    }
    
    class func toast(_ message: String?, duration: TimeInterval = FQToastManager.shared.duration, position: FQToastPosition = FQToastManager.shared.position, title: String? = nil, image: UIImage? = nil, style: FQToastStyle = FQToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
        parentView().makeToast(message, duration: duration, position: position, title: title, image: image, style: style, completion: completion)
    }
}
fileprivate extension UIView {
     ...
     */
    func makeToast(_ message: String?, duration: TimeInterval = FQToastManager.shared.duration, position: FQToastPosition = FQToastManager.shared.position, title: String? = nil, image: UIImage? = nil, style: FQToastStyle = FQToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
        do {
    ...
    /**
     ...
     didTap will be `true` if the toast view was dismissed from a tap.
     */
    func makeToast(_ message: String?, duration: TimeInterval = FQToastManager.shared.duration, point: CGPoint, title: String?, image: UIImage?, style: FQToastStyle = FQToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)?) {
        do {
            let toast = try toastViewForMessage(message, title: title, image: image, style: style)
    ...
    /**
     ...
     didTap will be `true` if the toast view was dismissed from a tap.
     */
    func showToast(_ toast: UIView, duration: TimeInterval = FQToastManager.shared.duration, position: FQToastPosition = FQToastManager.shared.position, completion: ((_ didTap: Bool) -> Void)? = nil) {
    ...
    /**
     ...
     didTap will be `true` if the toast view was dismissed from a tap.
     */
    func showToast(_ toast: UIView, duration: TimeInterval = FQToastManager.shared.duration, point: CGPoint, completion: ((_ didTap: Bool) -> Void)? = nil) {
        objc_setAssociatedObject(toast, &ToastKeys.completion, ToastCompletionWrapper(completion), .OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        
        if FQToastManager.shared.isQueueEnabled, activeToasts.count > 0 {
    ...
    /**
     ...
     @param position The toast's position
     */
    func makeToastActivity(_ position: FQToastPosition) {
        // sanity
    ...
    func hideToastActivity() {
        if let toast = objc_getAssociatedObject(self, &ToastKeys.activityView) as? UIView {
            UIView.animate(withDuration: FQToastManager.shared.style.fadeDuration, delay: 0.0, options: [.curveEaseIn, .beginFromCurrentState], animations: {
                toast.alpha = 0.0
            }) { _ in
    ...
    private func makeToastActivity(_ toast: UIView, point: CGPoint) {
        ...
        UIView.animate(withDuration: FQToastManager.shared.style.fadeDuration, delay: 0.0, options: .curveEaseOut, animations: {
            toast.alpha = 1.0
        })
    }
    
    private func createToastActivityView() -> UIView {
        let style = FQToastManager.shared.style
    ...
    private func showToast(_ toast: UIView, duration: TimeInterval, point: CGPoint) {
        ...
        if FQToastManager.shared.isTapToDismissEnabled {
        ...
        UIView.animate(withDuration: FQToastManager.shared.style.fadeDuration, delay: 0.0, options: [.curveEaseOut, .allowUserInteraction], animations: {
            toast.alpha = 1.0
        }) { _ in
     ...
     */
    func toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: FQToastStyle) throws -> UIView {
        // sanity
...
/**
 `FQToastStyle` instances define the look and feel for toast views created via the
 `makeToast` methods as well for toast views created directly with
 `toastViewForMessage(message:title:image:style:)`.
 
 @warning `FQToastStyle` offers relatively simple styling options for the default
 toast view. If you require a toast view with more complex UI, it probably makes more
 sense to create your own custom UIView subclass and present it with the `showToast`
 methods.
 */
public struct FQToastStyle {
...
/**
 `FQToastManager` provides general configuration options for all toast
 notifications. Backed by a singleton instance.
 */
public class FQToastManager {
    
    /**
     The `FQToastManager` singleton instance.
     
     */
    public static let shared = FQToastManager()
    ...
    /**
     ...
     Default is `FQToastPosition.Bottom`.
     
     */
    public var position: FQToastPosition = .bottom
    
}

// MARK: - FQToastPosition

public enum FQToastPosition {
    case top
    case center
    case bottom
    
    fileprivate func centerPoint(forToast toast: UIView, inSuperview superview: UIView) -> CGPoint {
        let topPadding: CGFloat = FQToastManager.shared.style.verticalPadding + superview.csSafeAreaInsets.top
        let bottomPadding: CGFloat = FQToastManager.shared.style.verticalPadding + superview.csSafeAreaInsets.bottom
